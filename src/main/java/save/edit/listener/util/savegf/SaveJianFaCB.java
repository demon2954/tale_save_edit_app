...
public class SaveJianFaCB extends AbstractSaveGongFaCheckBox {
	...
	public void saveM_MartialIDList(List<M_MartialIDList> martialIDList) {
		M_MartialIDList jianfaM_MartialIDList = null;
		if (null != martialIDList && martialIDList.size() > 0) {
			for (M_MartialIDList one : martialIDList) {
				if (one.getM_iType() == Constant.SKILL_TYPE_JIANFA) {
					jianfaM_MartialIDList = one;
					break;
				}
			}
		}
		List<M_BookNodeList> m_M_BookNodeListList = new ArrayList<M_BookNodeList>();
		if (jianfaM_MartialIDList == null) {
			jianfaM_MartialIDList = new M_MartialIDList();
			jianfaM_MartialIDList.setM_IDList(new ArrayList<String>());
			jianfaM_MartialIDList.setM_iType(Constant.SKILL_TYPE_JIANFA);
			jianfaM_MartialIDList.setM_BookNodeList(m_M_BookNodeListList);
		}
		...
		if (m_M_BookNodeListList != null) {
			jianfaM_MartialIDList.setM_BookNodeList(m_M_BookNodeListList);
			int jianfaType = jianfaM_MartialIDList.getM_iType();
			...
			martialIDList.add(jianfaM_MartialIDList);
		}
	}
}
